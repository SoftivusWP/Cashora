...
class Autoupdate
{
    ...
    public function checkUpdate($transient)
    {
        ...
        {
            $plugin_data = get_plugin_data(\CashbackTracker\PLUGIN_FILE);
            $res = new \stdClass();
            $res->slug = $this->slug;
    ...
    public function getRemoteInfo($false, $action, $arg)
    {
        ...
        if ($result && isset($result['info']))
        {
            ...
            $res->sections = array(
                'description' => sprintf(__('New version of %s plugin.', 'cashback-tracker'), $plugin_data['Name']) . ' ' .
                    sprintf(__('Please <a target="_blank" href="%s">find here</a> the releases notes.', 'cashback-tracker'), \esc_url(Plugin::getWebsite() . '/changelog/' . Plugin::getSlug() . '/readme.txt'))
            );
            return $res;
        }
        else
    ...
    {
        return Plugin::apiRequest(array('method' => 'POST', 'timeout' => 15, 'httpversion' => '1.0', 'blocking' => true, 'headers' => array(), 'body' => $this->getRequestArray($cmd), 'cookies' => array()));
    }
