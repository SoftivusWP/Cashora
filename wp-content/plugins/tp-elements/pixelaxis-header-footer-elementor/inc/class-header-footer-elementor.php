...
class Header_Footer_Elementor {
	...
	function __construct() {
		...
		if ( $is_elementor_callable ) {
			...
			if ( 'yes' === get_option( 'pixelaxis_hfe_plugin_is_activated' ) ) {
			...
			//add_action( 'tphfe_notice_before_markup_header-footer-elementor-rating', [ $this, 'rating_notice_css' ] );
		}
	...
	public function elementor_not_installed_activated() {
		...
		if ( ! did_action( 'elementor/loaded' ) ) {
			...
			$message = sprintf( __( 'The %1$sElementor Header & Footer Builder%2$s plugin requires %1$sElementor%2$s plugin installed & activated.', 'pixelaxis-header-footer-elementor' ), '<strong>', '</strong>' );
			...
			if ( _is_elementor_installed() ) {
				...
				$button_label = __( 'Activate Elementor', 'pixelaxis-header-footer-elementor' );

			} else {

				$action_url   = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=elementor' ), 'install-plugin_elementor' );
				$button_label = __( 'Install Elementor', 'pixelaxis-header-footer-elementor' );
			}
	...
	public function elementor_outdated() {
		...
		$class = 'notice notice-error';
		/* translators: %s: html tags */
		$message = sprintf( __( 'The %1$sElementor Header & Footer Builder%2$s plugin has stopped working because you are using an older version of %1$sElementor%2$s plugin.', 'pixelaxis-header-footer-elementor' ), '<strong>', '</strong>' );
		...
		if ( file_exists( WP_PLUGIN_DIR . '/elementor/elementor.php' ) ) {
			...
			$button_label = __( 'Update Elementor', 'pixelaxis-header-footer-elementor' );
	...
	public function show_setup_wizard() {
		...
		/* translators: %s: html tags */
		$notice_message = sprintf( __( 'Thank you for installing %1$s Elementor Header & Footer Builder %2$s Plugin! Click here to %3$sget started. %4$s', 'pixelaxis-header-footer-elementor' ), '<strong>', '</strong>', '<a href="' . $setting_url . '">', '</a>' );
	}
	...
	public function includes() {
		require_once TPHFE_DIR . 'admin/class-tphfe-admin.php';
		require_once TPHFE_DIR . 'admin/class-tphfe-settings.php';	

		require_once TPHFE_DIR . 'inc/tphfe-functions.php';

		// Load Elementor Canvas Compatibility.
		require_once TPHFE_DIR . 'inc/class-hfe-elementor-canvas-compat.php';

		// Load Target rules.
		require_once TPHFE_DIR . 'inc/lib/target-rule/class-tphf-target-rules-fields.php';
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'pixelaxis-header-footer-elementor' );
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'hfe-style', TPHFE_URL . 'assets/css/header-footer-elementor.css', [], TPHFE_VER );
	...
	public function enqueue_admin_scripts() {
		...
		if ( ( 'elementor-hf' == $screen->id && ( 'post.php' == $pagenow || 'post-new.php' == $pagenow ) ) || ( 'edit.php' == $pagenow && 'edit-elementor-hf' == $screen->id ) ) {
			...
			wp_enqueue_style( 'hfe-admin-style', TPHFE_URL . 'admin/assets/css/ehf-admin.css', [], TPHFE_VER );
			wp_enqueue_script( 'hfe-admin-script', TPHFE_URL . 'admin/assets/js/ehf-admin.js', [ 'jquery', 'updates' ], TPHFE_VER, true );
	...
	public function setup_unsupported_theme( $hfe_settings_tabs = [] ) {
		if ( ! current_theme_supports( 'pixelaxis-header-footer-elementor' ) ) {
			$hfe_settings_tabs['hfe_settings'] = [
				'name' => __( 'Theme Support', 'pixelaxis-header-footer-elementor' ),
		...
		if ( ! current_theme_supports( 'pixelaxis-header-footer-elementor' ) ) {
			...
			if ( '1' === $hfe_compatibility_option ) {
				if ( ! class_exists( 'HFE_Default_Compat' ) ) {
					require_once TPHFE_DIR . 'themes/default/class-hfe-default-compat.php';
				}
			} elseif ( '2' === $hfe_compatibility_option ) {
				require TPHFE_DIR . 'themes/default/class-global-theme-compatibility.php';
			}
		}
	}
	...
	public static function get_template_id( $type ) {
		...
		$hfe_templates = TPHF_Target_Rules_Fields::get_instance()->get_posts_by_conditions( 'elementor-hf', $option );
