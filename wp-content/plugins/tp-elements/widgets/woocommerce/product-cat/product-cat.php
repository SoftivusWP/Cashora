...
class Pixelaxis_Elementor_Product_Cat_Widget extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'TP Product Category', 'tp-elements' );
	}
	 ...
	 */
		...
		$this->add_control(
			'product_style',
			[
				'label'   => esc_html__( 'Product Category Style', 'tp-elements' ),
		...
		$this->add_control(
			'product_grid_source',
			[
				'label'   => esc_html__( 'Product Category Type', 'tp-elements' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'dynamic',				
				'options' => [
		...
		$this->add_control(
			'product_category',
			[
				'label'   => esc_html__( 'Select Product Category', 'tp-elements' ),
		...
		$this->add_control(
			'per_page',
			[
				'label' => esc_html__( 'Category Show Per Page', 'tp-elements' ),
	...
	protected function render() {
		...
		if ( class_exists( 'WooCommerce' ) ) {
			...
			$categories_limit = $settings['per_page'];
			...
			$tp_categories = get_terms( $args );

			if ( ! empty( $tp_categories ) && ! is_wp_error( $tp_categories ) ) {
			?>
			<div class="tp-product-wrapper tp-product-<?php echo esc_attr( $settings['product_style'] ); ?>">
				...
				<div class="tp-product-dynamic ">
					...
					<div class="swiper tp-product-cat-<?php echo esc_attr($unique); ?> ">
		...
		jQuery(document).ready(function(){
			var swiper<?php echo esc_attr($unique); ?><?php echo esc_attr($unique); ?> = new Swiper(".tp-product-cat-<?php echo esc_attr($unique); ?>", {				
				slidesPerView: 1,
				<?php echo $seffect; ?>
