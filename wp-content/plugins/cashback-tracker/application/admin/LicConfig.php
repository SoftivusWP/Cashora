...
class LicConfig extends Config
{
    ...
    public function add_admin_menu()
    {
        $this->resetLicense();
        $this->refreshLicense();
        \add_submenu_page(Plugin::slug, __('License', 'cashback-tracker') . ' &lsaquo; ' . Plugin::getName(), __('License', 'cashback-tracker'), 'manage_options', $this->page_slug(), array($this, 'settings_page'));
    }
    ...
    protected function options()
    {
        return array(
            'license_key' => array(
                'title' => __('License key', 'cashback-tracker'),
                'description' => __('Please enter a valid license key.', 'cashback-tracker') . ' ' . sprintf(__('You can find your key on the %s page.', 'cashback-tracker'), '<a href="' . \esc_url(Plugin::panelUri) . '" target="_blank">My Account</a>'),
                ...
                'validator' => array(
                    ...
                    array(
                        ...
                        'message' => sprintf(__('The field "%s" can not be empty.', 'cashback-tracker'), 'License key'),
                    ),
                    array(
                        'call' => array($this, 'licFormat'),
                        'message' => __('Invalid License key.', 'cashback-tracker'),
                    ),
                    array(
                        'call' => array($this, 'activatingLicense'),
                        'message' => __('License key is not accepted.', 'cashback-tracker') . ' ' . __('Please try again.', 'cashback-tracker') . ' ' . sprintf(__('If you are still having trouble with your License key please <a href="%s" target="_blank">contact</a> our support team.', 'cashback-tracker'), \esc_url(Plugin::supportUri)),
                    ),
                ),
            )
        );
    ...
    {
        if ($GLOBALS['pagenow'] != 'admin.php' || empty($_GET['page']) || $_GET['page'] != 'cashback-tracker-lic')
            return;
    ...
    private function refreshLicense()
    {
        ...
        if (isset($_POST['cbtrkr_cmd']) && $_POST['cbtrkr_cmd'] == 'refresh')
        {
