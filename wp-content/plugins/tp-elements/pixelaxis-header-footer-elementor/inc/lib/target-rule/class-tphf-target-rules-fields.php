 ...
 * Meta Boxes setup
 */
class TPHF_Target_Rules_Fields {
	/**
	 * Instance
	 *
	 * @since  1.0.0
	...
	public function __construct() {
		define( 'TP_TARGET_RULE_URI', plugins_url( '/', __FILE__ ) );
		define( 'TP_TARGET_RULE_VER', '1.0.0' );
		define( 'TP_TARGET_RULE_DIR', plugin_dir_path( __FILE__ ) );
	...
	public static function get_location_selections() {
		...
		$post_types = apply_filters( 'tphfe_location_rule_post_types', array_merge( $post_types, $custom_post_type ) );

		$special_pages = array(
			'special-404'    => __( '404 Page', 'pixelaxis-header-footer-elementor' ),
			'special-search' => __( 'Search Page', 'pixelaxis-header-footer-elementor' ),
			'special-blog'   => __( 'Blog / Posts Page', 'pixelaxis-header-footer-elementor' ),
			'special-front'  => __( 'Front Page', 'pixelaxis-header-footer-elementor' ),
			'special-date'   => __( 'Date Archive', 'pixelaxis-header-footer-elementor' ),
			'special-author' => __( 'Author Archive', 'pixelaxis-header-footer-elementor' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'pixelaxis-header-footer-elementor' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'pixelaxis-header-footer-elementor' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'pixelaxis-header-footer-elementor' ),
					'basic-singulars' => __( 'All Singulars', 'pixelaxis-header-footer-elementor' ),
					'basic-archives'  => __( 'All Archives', 'pixelaxis-header-footer-elementor' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'pixelaxis-header-footer-elementor' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'pixelaxis-header-footer-elementor' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'pixelaxis-header-footer-elementor' ),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'tphfe_display_on_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'pixelaxis-header-footer-elementor' ),
				'value' => array(
					'all'        => __( 'All', 'pixelaxis-header-footer-elementor' ),
					'logged-in'  => __( 'Logged In', 'pixelaxis-header-footer-elementor' ),
					'logged-out' => __( 'Logged Out', 'pixelaxis-header-footer-elementor' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'pixelaxis-header-footer-elementor' ),
				'value' => array(),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'tphfe_user_roles_list', $selection_options );
	}
	...
	public function admin_styles() {
		wp_enqueue_script( 'tphfe-select2', TP_TARGET_RULE_URI . 'select2.js', array( 'jquery' ), TP_TARGET_RULE_VER, true );
		...
		wp_register_script(
			'tphfe-target-rule',
			TP_TARGET_RULE_URI . 'target-rule.js',
			array(
				'jquery',
				'tphfe-select2',
			),
		...
		wp_enqueue_script( 'tphfe-target-rule' );

		wp_register_script(
			'tphfe-user-role',
			TP_TARGET_RULE_URI . 'user-role.js',
			array(
				'jquery',
			),
			TP_TARGET_RULE_VER,
		...
		wp_enqueue_script( 'tphfe-user-role' );

		wp_register_style( 'tphfe-select2', TP_TARGET_RULE_URI . 'select2.css', '', TP_TARGET_RULE_VER );
		wp_enqueue_style( 'tphfe-select2' );
		wp_register_style( 'tphfe-target-rule', TP_TARGET_RULE_URI . 'target-rule.css', '', TP_TARGET_RULE_VER );
		wp_enqueue_style( 'tphfe-target-rule' );
		...
		$localize_vars = array(
			'ast_lang'      => $ast_lang,
			'please_enter'  => __( 'Please enter', 'pixelaxis-header-footer-elementor' ),
			'please_delete' => __( 'Please delete', 'pixelaxis-header-footer-elementor' ),
			'more_char'     => __( 'or more characters', 'pixelaxis-header-footer-elementor' ),
			'character'     => __( 'character', 'pixelaxis-header-footer-elementor' ),
			'loading'       => __( 'Loading more results…', 'pixelaxis-header-footer-elementor' ),
			'only_select'   => __( 'You can only select', 'pixelaxis-header-footer-elementor' ),
			'item'          => __( 'item', 'pixelaxis-header-footer-elementor' ),
			'char_s'        => __( 's', 'pixelaxis-header-footer-elementor' ),
			'no_result'     => __( 'No results found', 'pixelaxis-header-footer-elementor' ),
			'searching'     => __( 'Searching…', 'pixelaxis-header-footer-elementor' ),
			'not_loader'    => __( 'The results could not be loaded.', 'pixelaxis-header-footer-elementor' ),
			'search'        => __( 'Search pages / post / categories', 'pixelaxis-header-footer-elementor' ),
			'ajax_nonce'    => wp_create_nonce( 'hfe-get-posts-by-query' ),
		);
		wp_localize_script( 'tphfe-select2', 'astRules', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'pixelaxis-header-footer-elementor' );
		$saved_values   = $value;
		$output         = '';
		...
		/* WP Template Format */
		$output .= '<script type="text/html" id="tmpl-tphfe-target-rule-' . $rule_type . '-condition">';
		$output .= '<div class="tp-target-rule-condition ast-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
		...
		$output .= '<option value="">' . __( 'Select', 'pixelaxis-header-footer-elementor' ) . '</option>';
		...
		$output .= '</div> <!-- tp-target-rule-condition -->';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'pixelaxis-header-footer-elementor' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'pixelaxis-header-footer-elementor' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'pixelaxis-header-footer-elementor' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="tp-target-rule-condition ast-target-rule-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			$output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="target_rule-condition-wrap" >';
		...
		if ( 'display' == $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'pixelaxis-header-footer-elementor' ) . '</a>';
			$output .= '</div>';
		}
	...
	public function get_current_layout( $layout_id, $option ) {
		...
		$current_layout = apply_filters( 'tphfe_target_page_settings', $current_layout, $layout_id );
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		$output         .= '<script type="text/html" id="tmpl-tphfe-user-role-condition">';
			$output     .= '<div class="tphfe-user-role-condition ast-user-role-{{data.id}}" data-rule="{{data.id}}" >';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			$output .= '</optgroup>';
					...
					$output .= '</select>';
				$output     .= '</div>';
			$output         .= '</div> <!-- tphfe-user-role-condition -->';
		$output             .= '</script>';
		...
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="tphfe-user-role-condition ast-user-role-' . $index . '" data-rule="' . $index . '" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( is_array( self::$current_page_data ) && isset( self::$current_page_data[ $post_type ] ) ) {
			return apply_filters( 'tphfe_get_display_posts_by_conditions', self::$current_page_data[ $post_type ], $post_type );
		}
		...
		if ( ! empty( $already_set_rule ) ) {
			add_action(
				...
				function() use ( $already_set_rule ) {
					...
					$notice = sprintf( __( 'The same display setting is already exist in %s post/s.', 'pixelaxis-header-footer-elementor' ), $rule_set_titles );
	...
	public static function get_post_selection( $post_type ) {
		...
		if ( ! empty( $all_headers ) ) {
			$headers = array(
				'' => __( 'Select', 'pixelaxis-header-footer-elementor' ),
			);
 ...
 */
TPHF_Target_Rules_Fields::get_instance();
