...
*/
class pixelaxis_soical_Widget extends WP_Widget {
	...
	*/
	function __construct() {
		parent::__construct(
			'pixelaxis_soical_Widget', // Base ID
			esc_html__( 'TP Social Icon Widget', 'tp-framework' ), // Name
			array( 'description' => esc_html__( 'TP Social icon widget area', 'tp-framework' ), ) // Args
		);
	}
	...
	*/
	public function field_generator( $instance ) {
		...
		foreach ( $this->widget_fields as $widget_field ) {
			$widget_value = ! empty( $instance[$widget_field['id']] ) ? $instance[$widget_field['id']] : esc_html__( isset($widget_field['default']), 'tp-framework' );
			switch ( $widget_field['type'] ) {
				default:
					$output .= '<p>';
					$output .= '<label for="'.esc_attr( $this->get_field_id( $widget_field['id'] ) ).'">'.esc_attr( $widget_field['label'], 'tp-framework' ).':</label> ';
	...
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( '', 'tp-framework' );
		?>
		<p>
		<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title:', 'tp-framework' ); ?></label>
...
} // class pixelaxis_soical_Widget

// register medvill Social Icon Widget widget
function register_pixelaxis_soical_Widget() {
	register_widget( 'pixelaxis_soical_Widget' );
}
add_action( 'widgets_init', 'register_pixelaxis_soical_Widget' );